...
/**
 * TODO: FINISH! @triszt4n
 * THIS FILE IS NOT YET FINISHED, USED THE NEXTUI WEBSITE AS A REFERENCE (BASICALLY A COPY).
 * WILL NEED TO ATTRIBUTE THIS FILE TO NEXTUI.
 */
...
export const Cmdk: FC<{}> = () => {
  ...
  const onInputKeyDown = useCallback(
    ...
    [activeItem, onItemSelect, recentSearches, results],
  )
  ...
  const renderItem = useCallback(
    (item: SearchResultItem, index: number, isRecent = false) => {
      ...
      const mainIcon = isRecent ? (
        <MagnifyingGlassIcon className={slots.leftIcon()} strokeWidth={2} />
      ) : (
        <LinkIcon className={slots.leftIcon()} />
      )
    ...
    [slots, menuNodes, activeItem, onItemSelect],
  )
